...
public class WordCo4a
{
			...
			for (int x=0;x<word1.length;x++) 
			{
				for(int y=x;y<word1.length;y++)
				{
					if(y!=x)
					{
						...
						word1[y]=word1[y].replaceAll("j","i");
						word1[y]=word1[y].replaceAll("v","u");
						

						//check the lemmatizer for normalized spelling
						if(hm.containsKey(word1[x]) && hm.containsKey(word1[y]))
						{
							String temp=hm.get(word1[x]).toString()+","+hm.get(word1[y]);
							//	System.out.println(temp);
							w=new Text(temp);
							v=new Text(text[0]);
							context.write(w,v);
						}
						else if(hm.containsKey(word1[x]) && !hm.containsKey(word1[y]))
						{
							String temp=hm.get(word1[x]).toString()+","+word1[x];
							//	System.out.println(temp);
							w=new Text(temp);
							v=new Text(text[0]);
							context.write(w,v);
						...
						{
							String temp=word1[x]+","+hm.get(word1[y]).toString();
							//	System.out.println(temp);
							w=new Text(temp);
							v=new Text(text[0]);
							context.write(w,v);
	...
	//	conf.set("lemmas",sc);
    		...
    		job.setJarByClass(WordCo4a.class);
